...
//        self.mainWindow = [self windowWithLevel:UIWindowLevelNormal];
        ...
        self.alertView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithWhite:0.25 alpha:1];
...
- (UIButton *)genericButton
{
    ...
    [button setTitleColor:WAWA_COLOR_RED forState:UIControlStateNormal];
...
{
    [sender setBackgroundColor:GROUP_TABLE_BACKGROUND_COLOR];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    if ([[[PXAlertViewStack sharedInstance] alertViews] count]) {
        if (animated) {
...
- (CALayer *)lineLayer
{
    ...
    lineLayer.backgroundColor = [[UIColor colorWithRed:233/255.0 green:233/255.0 blue:233/255.0 alpha:1] CGColor];
    return lineLayer;
}
